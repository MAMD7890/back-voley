"""
Lambda: galacticos-membresia-core-job2-detectar-vencimientos
Trigger: EventBridge cron(0 5 * * ? *) → medianoche hora Colombia (UTC-5)

Job 2 — DetectarVencimientos
Busca membresías PAGADA con fechaFin < hoy. Si el estudiante no tiene otra
membresía activa, marca la vencida como FINALIZADA y crea una nueva tipo MORA
con 15 días de gracia.
...
ENDPOINT = f"{BACKEND_URL}/api/internal/membresias-core/jobs/detectar-vencimientos"
TIMEOUT_SEG = 60


def lambda_handler(event, context):
    logger.info("Job 2 — DetectarVencimientos iniciando...")
    ...
    try:
        with urllib.request.urlopen(req, timeout=TIMEOUT_SEG) as resp:
            ...
            logger.info(
                "morasCreadas=%s omitidas=%s errores=%s",
